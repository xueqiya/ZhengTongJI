package com.pintu.deutils.util;
...
public class GameUtil {
    ...
    public static void getPuzzleGenerator() {
        int index = 0;
        // 随机打乱顺序
        Bitmap blankBitmap = mItemBeans.get(PuzzleMain.TYPE * PuzzleMain.TYPE-1).getBitmap();
        mItemBeans.remove(PuzzleMain.TYPE * PuzzleMain.TYPE - 1);
        for (int i = 0; i < mItemBeans.size(); i++) {
            index = (int) (Math.random() *
                    PuzzleMain.TYPE * PuzzleMain.TYPE-1);
            swapItems(mItemBeans.get(index), mItemBeans.get(1));
        }
        GameUtil.mItemBeans.add(new ItemBean(PuzzleMain.TYPE * PuzzleMain.TYPE, 0, blankBitmap));
        GameUtil.mBlankItemBean = GameUtil.mItemBeans.get(PuzzleMain.TYPE * PuzzleMain.TYPE - 1);
