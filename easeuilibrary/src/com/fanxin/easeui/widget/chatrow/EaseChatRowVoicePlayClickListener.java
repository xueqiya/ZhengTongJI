...
public class EaseChatRowVoicePlayClickListener implements View.OnClickListener {
	...
	public EaseChatRowVoicePlayClickListener(EMMessage message, ImageView v, ImageView iv_read_status, BaseAdapter adapter, Activity context, int position, EMConversation conversation) {
			...
			mediaPlayer.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
				...
				public void onCompletion(MediaPlayer mp) {
					...
					if(message.equals(emMessage_last)){   //  判断如果是最后一条语音则不做处理
						...
						if (emMessage1 != null) {
							if (type == EMMessage.Type.VOICE.ordinal() && message.direct() == EMMessage.Direct.RECEIVE&& !emMessage1.isAcked()) { // 判断是否是接收到语音
								...
								while (!emMessages.get(pos).equals(emMessage_last)){
									pos++;
									EMMessage emMessage2 = emMessages.get(pos);
									int type2 = emMessage2.getType().ordinal();
									if (type2 == EMMessage.Type.VOICE.ordinal() && emMessage2.direct() == EMMessage.Direct.RECEIVE&& !emMessage2.isAcked()) { // 判断是否是接收到语音
										EaseChatRowVoice easeChatRowVoice = new EaseChatRowVoice(activity, emMessage2, pos, adapter, conversation);
										easeChatRowVoice.onBubbleClick();
										break;
									}
								}
